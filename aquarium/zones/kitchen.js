// aquarium/zones/kitchen.js — items that live in the kitchen surrounding
// the bowl. Two groups, all tagged `regionKey: 'kitchen'`:
//
//   - structural: room walls (an inverted box enclosing everything), the
//     chessboard floor, the ceiling plane, and the potlights cluster.
//   - props: table + runner, glass vase with flower, full-height fridge
//     with the kid's sailboat drawing, counter spanning the back wall,
//     sink + faucet beneath the window, big window with frame + sill,
//     framed Dutch-masters maritime painting, and a tall closed door on
//     the opposite wall.
//
// A bowl-region ray traveling out the open top crosses into the kitchen
// region (per-step regionFn) before it can reach any of these structural
// items, so tagging them `kitchen` is safe — they become considered for
// rendering the moment a step lands in kitchen. Same for horizontal rays
// passing through the translucent bowl glass.
...
// Vase — hollow cylindrical glass cup. Outer cylinder minus an inner
// cylinder whose top extends far above the outer, so cutSDF reports correct
// distances above the rim and physics doesn't push the fish off a phantom lid.
